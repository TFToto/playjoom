...
class JCategories
{
	...
	public function __construct($options) {
		...
		$this->_extension = (isset($options['extension']) && $options['extension']) ? $options['extension'] : 'com_playjoom';
		$this->_table = (isset($options['table']) && $options['table']) ? $options['table'] : '#__jpaudiotracks';
		$this->_field = (isset($options['field']) && $options['field']) ? $options['field'] : 'id';
		...
		if (isset($this->_options['countItems']) && $this->_options['countItems'] == 1)
		{
			if ($this->_options['published'] == 1)
			{
				$query->join(
					...
					$db->quoteName($this->_table) . ' AS i ON i.' . $db->quoteName($this->_field) . ' = c.id'
