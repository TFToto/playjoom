 ...
 * PlayJoom Model
 */
class PlayJoomModelDownload extends JModelList {
    ...
    protected function getListQuery() {
	    ...
	    switch($item_source) {
	    	...
	    	case "album" :
			    ...
			    $this->getState(
				                'list.select',
				                 'a.id, a.length, a.pathatlocal, a.file, a.album, a.artist, a.tracknumber, a.title, a.mediatype, a.length'
			                   )
		                      );
	        ...
	        case "artist" :
			    ...
			    $this->getState(
				                'list.select',
				                 'a.id, a.title, a.artist, a.album, a.length, a.tracknumber, a.year'
			                   )
		                      );
		        $query->from('#__jpaudiotracks AS a');

		        $query->where('a.artist = "'.base64_decode(JRequest::getVar('name')).'"');
	        ...
	        case "playlist" :
		        ...
		        $query->select('a.id, a.title, a.artist, a.album, a.length, a.tracknumber, a.year, a.hits, a.pathatlocal, a.file, a.mediatype');
	        ...
	        case "tag" :
	        	...
	        	if (JRequest::getVar('listid')) {
	        		$query->select(
	        				$this->getState(
	        						'list.select',
	        						'a.id, a.title, a.artist, a.album, a.length, a.tracknumber, a.year, a.hits, a.pathatlocal, a.file, a.mediatype'
	        				)
	        		);
	        ...
	        case "genre" :
			    ...
			    $this->getState(
				                'list.select',
				                 'a.id, a.title, a.artist, a.album, a.length, a.tracknumber, a.year, a.catid'
			                   )
		                      );
		        $query->from('#__jpaudiotracks AS a');
