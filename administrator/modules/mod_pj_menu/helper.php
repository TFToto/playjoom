...
abstract class ModMenuHelper
{
	...
	public static function getComponents($authCheck = true)
	{
		...
		$query->where('m.id > 1');
		$query->where('m.title != "com_playjoom"');
	...
	/**
	 * Get a list of the authorised, non-special PlayJoom parts to display in the Content menu.
	 *
	 * @param	boolean	$authCheck	An optional switch to turn off the auth check (to support custom layouts 'grey out' behaviour).
	 *
	 * @return	array	A nest array of component objects and submenus
	 * @since	1.7
	 */
	public static function getPlayJoomParts($authCheck = true) {
		...
		// Prepare the query.
		$query->select('m.id, m.title, m.alias, m.link, m.level, m.parent_id, m.img, e.element');
		$query->from('#__menu AS m');
		...
		$query->where('e.enabled = 1');
		$query->where('m.id > 1');
		$query->where('e.name = "playjoom"');
		//$query->Where('m.level = 2');
