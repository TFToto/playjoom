...
{
	$menu->addChild(
		new JMenuNode(JText::_('MOD_MENU_SYSTEM_INFORMATION'), 'index.php?option=com_playjoomadmin&view=sysinfo', 'class:info')
	);
}
...
if ($user->authorise('core.manage', 'com_playjoom'))
{
	// Get the PlayJoom Components items as sub-menus.
	$components = ModMenuHelper::getPlayJoomParts( true );

	if ($components) {
		$menu->addChild(new JMenuNode(JText::_('MOD_MENU_COM_PLAYJOOM_MANAGE'), '#'), true);
