			...
			if (isset($DirectoryContents[$dirname]['dir']) && is_array($DirectoryContents[$dirname]['dir'])) {
				...
				foreach ($DirectoryContents[$dirname]['dir'] as $filename => $fileinfo) {
					...
					if ($filename == '..') {
						...
						echo '<form action="'.JRoute::_('index.php?option=com_playjoom&view=cover&layout=edit').'" method="post">';
						echo '<a href="'.JRoute::_('index.php?option=com_playjoom&view=cover&layout=edit&listdirectory='.$session->get('backstep')).'"><img src="components/com_playjoom/images/j_button1_prev.png" alt="back" /></a>
                              <a href="'.JRoute::_('index.php?option=com_playjoom&view=cover&layout=edit&listdirectory='.DIRECTORY_SEPARATOR).'"><img src="components/com_playjoom/images/j_button1_home.png" alt="home" /></a>
						      &nbsp;&nbsp;Parent directory: ';
						...
						echo '<td colspan="'.$columnsintable.'"><img src="components/com_playjoom/images/folder.png" alt="folder" />&nbsp;&nbsp;<a href="'.JRoute::_('index.php?option=com_playjoom&view=cover&layout=edit&listdirectory='.urlencode($dirname.$filename)).'"><b>'.$filename.'</b></a></td>';
					}
					echo '</tr>';
				}
			}
			if (isset($DirectoryContents[$dirname]['known']) && is_array($DirectoryContents[$dirname]['known'])) {
				...
				foreach ($DirectoryContents[$dirname]['known'] as $filename => $fileinfo) {
					...
					switch($fileinfo['fileformat']) {
					    ...
					    break;
					    case 'jpg':
							$rowentry = '<img src="components/com_playjoom/images/picture.png" alt="picture file" />&nbsp;&nbsp;<a href="'.JRoute::_('index.php?option=com_playjoom&view=cover&layout=edit&coverpath='.base64_encode(realpath($listdirectory).DIRECTORY_SEPARATOR.$filename)).'">'.$filename.'</a>';
					    break;
					    default:
