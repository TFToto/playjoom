 ...
 * @since       0.9
 */
class PlayjoomupdateViewCopyFiles extends JViewLegacy {
	...
	function display($tpl = null)	{
		...
		$files_array = explode('*|*', JFactory::getApplication()->getUserState('com_playjoomupdate.filespaths.array'));

		if (count($files_array) > 1) {
			$this->total = count($files_array);
		} else {
			$this->total = 1;
		}
	...
	protected function setDocument() {
		...
		$js = "jQuery(function(){
				...
				function load() {
					jQuery.ajax({
						url: 'index.php?option=com_playjoomupdate&task=update.runningcopy&format=raw&total=".$this->total."&oneprocent=".$this->one_procent."&status='+jQuery( \"#progressbar\" ).progressbar( \"value\" ),
						success: function(data) {
							...
							if(ajax!=false) {
								...
								if(ajax.status<100) {
				                    ...
				                    window.location = 'index.php?option=com_playjoomupdate&task=update.extensionssetup';
				                }
							}
						}
					});
