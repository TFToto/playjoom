<?php
/**
 * Contains the download helper methods for PlayJoom Update.
 *
...
class AdmintoolsHelperDownload
{
	...
	private static function &getCURL($url, $fp = null, $nofollow = false)
	{
		...
		curl_setopt($ch, CURLOPT_USERAGENT, 'Joomla/' . PJVERSION);
	...
	private static function &getFOPEN($url, $fp = null)
	{
		...
		{
			$httpopts = array('user_agent' => 'PlayJoom/' . PJVERSION);
		...
		else
		{
			...
			if ( function_exists('ini_set') )
			{
				ini_set('user_agent', 'PlayJoom/' . PJVERSION);
			}
			$ih = @fopen($url, 'r');
		}
