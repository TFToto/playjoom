<?php
/**
 * Contains the plugin methods for to get the list of results for artist sarching in PlayJoom frontend.
 *
 ...
 * @since		0.9.473
 */
class plgSearchPj_artists extends JPlugin
{
	...
	{
		static $areas = array(
			'artists' => 'PLG_SEARCH_ARTISTS_ARTISTS'
			);
			return $areas;
	}
	...
	function onContentSearch($text, $phrase='', $ordering='', $areas=null)
	{
		...
		$sContent		= null;
		$sArchived		= null;
		$limit			= $this->params->def('pj_artists_search_limit',		50);
		...
		$section	= JText::_('PLG_SEARCH_ARTISTS');
		...
		$query->clear();
		$query->select('a.id, a.catid, a.pathatlocal, a.file, a.title AS tracktitle, a.album AS albumname, a.artist AS artistname, a.metadesc, a.metakey, a.add_datetime AS created, '
		...
		if (isset($list))
		{
			...
			{
				$list[$key]->href = 'index.php?option=com_playjoom&view=artist&artist='.base64_encode($item->artistname);
				$list[$key]->title = $item->artistname;
