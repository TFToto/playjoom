<?php
/**
 * Contains the plugin methods for to get the list of results for track sarching in PlayJoom frontend.
 *
...
/**
 * PJ_Tracks Search plugin
 *
 * @package	PlayJoom.Plugin
 * @subpackage Search.pj_tracks
 * @since		0.9.473
 */
class plgSearchPj_tracks extends JPlugin
{
	...
	{
		static $areas = array(
			'tracks' => 'PLG_SEARCH_TRACKS_TRACKS'
			);
			return $areas;
	}
	...
	function onContentSearch($text, $phrase='', $ordering='', $areas=null)
	{
		...
		$sContent		= null;
		$sArchived		= null;
		$limit			= $this->params->def('pj_tracks_search_limit',		50);
		...
		$section	= JText::_('PLG_SEARCH_TRACKS');
		...
		if (isset($list))
		{
			...
			{
				$list[$key]->href = 'index.php?option=com_playjoom&view=broadcast&id='.$item->id;
				$list[$key]->title = $item->artistname.' | '.$item->tracktitle.' ('.$item->albumname.')';
