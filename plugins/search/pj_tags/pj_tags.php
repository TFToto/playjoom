 ...
 * @since       3.3
 */
class PlgSearchPJ_tags extends JPlugin
{
	/**
	...
	public function onContentSearch($text, $phrase = '', $ordering = '', $areas = null) {
		...
		$section = JText::_('PLG_SEARCH_PJ_TAGS_TAGS_SECTION');
		...
		if ($rows) {
			...
			foreach ($rows as $key => $row) {
				$rows[$key]->href = 'index.php?option=com_playjoom&view=tag&id='.$row->id.':'.$row->title;
		...
		if (!$this->params->get('show_tagged_items')) {
			...
			foreach ($rows as $key => $row) {
				...
				if ($tagged_items) {
					...
					//echo print_r($tagged_items);
					foreach ($tagged_items as $k => $item) {
						$new_item = new stdClass;
						$new_item->href = 'index.php?option=com_playjoom&view=broadcast&id='.$item->content_item_id;;
