...
echo '<fieldset class="batch">';
	...
	echo '<form action="'.JRoute::_('index.php?option=com_playjoom&view=playlist').'" method="post" name="playlistForm" id="playlistForm">';
	     ...
	     echo '<select name="orderplaylist" class="PJ-filtermenu" id="ordertype" onchange="this.form.submit()">';
	         ...
	         echo '<option value="a.tracknumber,a.year">'.JText::_('COM_PLAYJOOM_PLAYLIST_ORDER_TYPE_TRACKNO').'</option>';
	         ...
	         echo '<option value="a.hits,a.year">'.JText::_('COM_PLAYJOOM_PLAYLIST_ORDER_TYPE_HITS').'</option>';
	     echo '</select>';
	     
	     echo '<input type="hidden" name="name" value="'.JRequest::getVar('album').'" />';
	     echo '<input type="hidden" name="source" value="artist" />';
