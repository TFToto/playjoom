...
<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm" class="form-inline">
	...
	<ul class="side-nav category list-striped">
			...
			<?php else: ?>
				...
				<h5>
					<a href="<?php echo JRoute::_(TagsHelperRoute::getItemRoute($item->content_item_id, $item->core_alias, $item->core_catid, $item->core_language, $item->type_alias, $item->router)); ?>">
						<?php echo sprintf ("%02d",$i +1) .' - '.$this->escape($item->core_title); ?>
					</a>
				</h5>
			<?php endif; ?>
...
$track_text = '<span class="trackno">'.$NoLink.'</span> - '.$this->events->BeforeTrackLink.'<span class="tracktitle">'.$TitleLink.'</span>&nbsp;<span class="trackminutes"></span>'.$this->events->AfterTrackLink.' <span class="add2playlist"><a href="index.php?option=com_playjoom&amp;view=addtoplaylist&amp;layout=modal&amp;tmpl=component&amp;id='.$item->id.'" class="modal" style="margin-left: 45px;" rel="'.$modal_add2playlist_config.'">'.JText::_('COM_PLAYJOOM_ALBUM_ADD2PLAYLIST').'</span></a>';
echo $track_text;
