	...
	foreach ($this->t['categories'] as $value) {
		...
		foreach ($value->subcategories as $valueCat) {
			...
			if ($rightDisplay == 1) {
				...
				$catOutput 	.= '<li class="bullet-item">';
				$catOutput 	.= '<i class="fa fa-folder-o"></i> <a href="'. JRoute::_(PhocaDownloadRoute::getCategoryRoute($valueCat->id, $valueCat->alias))
							.'">'. $valueCat->title.'</a>';
			
				if ($this->t['displaynumdocsecs'] == 1) {
					$catOutput  .=' <span class="badge badge-info tip">'.$valueCat->numdoc .'</span>';
				}
				$catOutput 	.= '</li>' . "\n";
...
// =====================================================================================
			...
			$pdTitle = '<i class="fa fa-folder"></i> <a href="'. JRoute::_(PhocaDownloadRoute::getCategoryRoute($value->id, $value->alias)).'">'. $value->title.'</a>';
			...
			if ($this->t['displaynumdocsecsheader'] == 1) {
				...
				$pdTitle .= ' <span class="badge badge-info tip">'.$numSubcat.'/' . $numDocAll .'</span>';
			}
			...
			if ($this->t['displaymaincatdesc']	 == 1) {
				...
				if ($catOutput != '') {
					...
					$pdSubcategories .= '<li class="description">'.JText::_('COM_PHOCADOWNLOAD_NO_SUBCATEGORIES').'</li>';
				}
			}
			...
			if ($this->t['display_specific_layout'] == 0) {
				...
				echo '<li class="title">'.$pdTitle.'</li>';
				if ($pdDesc != '') { echo '<li class="description">'.$pdDesc.'</li>';}
...
if (!empty($this->t['mostvieweddocs']) && $this->t['displaymostdownload'] == 1) {
	...
	foreach ($this->t['mostvieweddocs'] as $value) {
		...
		if ($rightDisplay == 1) {
			...
			$outputFile .= '<li class="bullet-item" '
					. $imageFileName['filenamestyle'].'>';
			
			$outputFile .= '<a href="'
						. JRoute::_(PhocaDownloadRoute::getCategoryRoute($value->categoryid,$value->categoryalias))
						.'">'. $value->title.'</a>'
						.' <span class="badge badge-info tip"> | <i class="fa fa-folder"></i> ' .$value->categorytitle.'</span>';
