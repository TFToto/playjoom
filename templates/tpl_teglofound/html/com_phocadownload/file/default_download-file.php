...
echo '<ul class="pricing-table">';
	...
	if (!empty($this->category[0])) {
		if ($this->t['display_up_icon'] == 1 && $this->t['tmplr'] == 0) {
			if (isset($this->category[0]->id)) {
				...
				echo '<a title="'.$linkUpText.'" href="'. $linkUp.'" ><i class="fa fa-undo"></i></a>';
			}
		}
	}
	...
	if ((int)$this->t['display_file_view'] == 1
		...
		if ($fileSize != '') {
			$pdFileSize .= '<li class="bullet-item">'.JText::_('COM_PHOCADOWNLOAD_FILESIZE').': '.$fileSize.'</div>';
		}
		...
		if ($l->isValueEditor($v->description)) {
			$pdDescription .= '<li class="description">'.$v->description.'</li>';
		}
